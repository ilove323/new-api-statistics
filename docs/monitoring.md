...
- 设置窗口分为“余额设置”和“报警渠道”两个标签页。余额设置中的消费渠道来自 New API 当前渠道列表；勾选的渠道计入预算，取消勾选的渠道不计入。
- 监控库只保存被排除的渠道 ID，不复制 New API 渠道资料。每次打开设置页都会重新读取渠道列表，因此新增渠道默认勾选并计入统计。
...
- 渠道选择立即影响本月现场查询和以后补齐的缺失月份，但不改写已有月度归档。需要追溯时必须显式重新统计目标月份，系统不会自动覆盖历史快照。
- “追溯历史计费”需要连续三次确认，并按照页面当前渠道规则重算累计起始月份至上月的已有归档。系统先只生成逐月的原费用、新费用和差额对比，不写数据库；管理员确认对比后才提交。提交时会再次计算，并要求设置版本、旧归档和新费用与预览完全一致，否则拒绝写入并要求重新预览。追溯前还会逐月比较 New API 当前原始金额与旧归档；缺少任一旧归档，或当前原始金额低于旧归档时，视为历史日志不完整并回滚整个操作，旧归档和原渠道设置均保留。由于源库没有逐条归档快照，该校验只能识别可由旧归档金额证明的数据缺失，不能证明源日志绝对完整。
