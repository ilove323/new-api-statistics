...
- “追溯历史计费”需要连续三次确认，并按照页面当前渠道规则重算累计起始月份至上月的已有归档。追溯前逐月比较 New API 当前原始金额与旧归档；缺少任一旧归档，或当前原始金额低于旧归档时，视为历史日志不完整并回滚整个操作，旧归档和原渠道设置均保留。由于源库没有逐条归档快照，该校验只能识别可由旧归档金额证明的数据缺失，不能证明源日志绝对完整。
