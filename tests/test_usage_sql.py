    ...
    def test_site_model_convention_and_latest_ratio(self):
        ...
        for model, semantic, p, c, cr, cw, _, _ in cases:
            for index, ratio in enumerate((3, 3, 3.4), 1):
                ...
                fixtures.append(dict(id=index, created_at=index, user_id=1, username='fixture',
                                     model_name=model, quota=100, prompt_tokens=p,
                                     completion_tokens=c, other=json.dumps(other), type=2))
        prefix = """WITH logs AS (
        ...
        with psycopg.connect(connect_timeout=8, row_factory=dict_row,
            ...
            rows = conn.execute(query, dict(fixtures=json.dumps(fixtures), start=0, end=10)).fetchall()
