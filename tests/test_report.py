...
from app import app
from report import current_prices, decorate, export_excel, period, totals, rankings, TOKEN_FIELDS, convert_tokens, load_site_name, cost_formula


class ReportTest(unittest.TestCase):
    def test_cost_formula_decimal_and_difference(self):
        row=dict(input_tokens=1000000,output_tokens=200000,cache_read_tokens=300000,
                 cache_write_tokens=100000,input_price=Decimal(2),output_price=Decimal(10),
                 cache_price=Decimal('.2'),write_price=Decimal('2.5'),group_ratio=Decimal(3),
                 amount=Decimal('13'),ratio_count=2,converted_cache_read_tokens=None)
    ...
    def test_site_name_database_and_fallback(self):
        with patch('report.psycopg.connect') as connect:
            conn = connect.return_value.__enter__.return_value
            for value, expected in [(('示例网关',), '示例网关'), (None, 'New API'), (('',), 'New API')]:
    ...
    def test_site_name_rendered_and_escaped(self):
        with patch('app.verify_admin', return_value=True), patch('app.load_site_name', return_value='示例网关<script>'):
    ...
    def setUp(self):
        self.rows = decorate([
            dict(user_id=1, username='=danger', display_name='测试显示名', model_name='claude-a', total_tokens=100, request_count=2,
                 input_tokens=10, output_tokens=20, cache_write_tokens=30, cache_read_tokens=40,
                 amount=Decimal('10'), group_ratio=Decimal('3.8')),
            dict(user_id=1, username='=danger', display_name='测试显示名', model_name='gpt-b', total_tokens=900, request_count=4,
                 input_tokens=800, output_tokens=100, cache_write_tokens=0, cache_read_tokens=500,
                 amount=Decimal('1'), group_ratio=Decimal('1')),
        ], {})
    ...
    def test_token_breakdown(self):
        ...
        self.assertEqual(set(result), {*TOKEN_FIELDS, 'total_tokens', 'amount', 'users', 'models', 'request_count'})
    ...
    def test_rankings_aggregate_users_before_sorting(self):
        other = dict(self.rows[0], user_id=2, username='other', amount=Decimal('20'), total_tokens=50)
        result = rankings([*self.rows, other])
        self.assertEqual([r['username'] for r in result['user_tokens']], ['=danger','other'])
        self.assertEqual([r['username'] for r in result['user_amount']], ['other','=danger'])
    ...
    def test_excel_exact_time(self):
        ws=load_workbook(export_excel([], '2026-07-26T18:20:30','2026-07-26T19:21:31')).active
        self.assertEqual(ws['B1'].value,'2026-07-26 18:20:30 至 2026-07-26 19:21:31（北京时间）')

    def test_prices_missing_not_zero(self):
        options = {'ModelRatio': {'claude-a': 5}, 'CompletionRatio': {'claude-a': 5},
                   'CacheRatio': {'claude-a': 0.1}}
        self.assertEqual(current_prices('claude-a', options),
    ...
    def test_merged_conversion_used_consistently(self):
        original=dict(user_id=2,username='converted',model_name='gpt',request_count=2,
                      total_tokens=10000,input_tokens=10000,output_tokens=0,
                      cache_read_tokens=10000,cache_write_tokens=0,pricing_input_tokens=0,
                      ratio_count=2,group_ratio=Decimal('3.4'),amount=Decimal('.032'))
        options={'ModelRatio':{'gpt':1},'CacheRatio':{'gpt':.5}}
        ...
        self.assertEqual(rankings([row])['user_tokens'][0]['total_tokens'],row['total_tokens'])
    ...
    def test_conversion_unique_solution_preserves_original(self):
        row=dict(ratio_count=2, group_ratio=Decimal('3.4'), amount=Decimal('0.032'),
                 pricing_input_tokens=0, output_tokens=0, cache_write_tokens=0,
                 cache_read_tokens=10000, total_tokens=10000,
                 input_price=None, output_price=None, write_price=None, cache_price=Decimal(1))
        convert_tokens(row)
        self.assertLess(row['converted_cache_read_tokens'],10000)
        self.assertEqual(row['cache_read_tokens'],10000)
        self.assertEqual((row['converted_cache_read_tokens']*row['cache_price']*row['group_ratio']/1000000).quantize(Decimal('.000001')),row['amount'])
        row['group_ratio']=Decimal(3)
        convert_tokens(row)
        self.assertGreater(row['converted_cache_read_tokens'],10000)
        self.assertEqual(row['converted_total_tokens'],row['converted_cache_read_tokens'])
        for count in (1,3):
    ...
    def test_excel_ranking_sheets(self):
        ...
        self.assertEqual(wb.sheetnames, ['用户模型用量','模型消费','用户Token用量','用户消费','区间汇总'])
    ...
    def test_developer_mode_does_not_change_excel(self):
        with patch('app.verify_admin', return_value=True), patch('app.load_report', return_value=self.rows):
            books=[]
            for suffix in ['', '&dev=0', '&dev=1']:
                response=app.test_client().get('/statistics/api/export?start=2026-07-26&end=2026-08-25'+suffix,
    ...
    def test_auth_and_user_filter(self):
        with patch('app.verify_admin', side_effect=lambda u,p: u == 'test_admin' and p == 'testing'):
            client = app.test_client()
            self.assertEqual(client.get('/statistics/api/usage').status_code, 401)
            with patch('app.load_report', return_value=deepcopy(self.rows)):
                response = client.get('/statistics/api/usage?start=2026-07-26&end=2026-08-25', auth=('test_admin','testing'))
                self.assertEqual(response.status_code, 200)
                self.assertEqual(response.json['totals']['amount'], '11')
                self.assertEqual(response.json['totals']['request_count'], 6)
                response = client.get('/statistics/api/usage?user=absent&start=2026-07-26&end=2026-08-25', auth=('test_admin','testing'))
                self.assertEqual(response.json['rows'], [])
                self.assertEqual(response.json['totals']['rpm'], '0')
            self.assertEqual(client.get('/statistics/', auth=('admin','testing')).status_code, 401)
