...
from notification_channels import feishu_app
...
def main():
    ...
    def connect(*args,**kwargs):
        return original_connect(dsn,row_factory=dict_row,options='-c search_path='+schema)
    with original_connect(dsn) as conn:
        conn.execute(sql.SQL('CREATE SCHEMA {}').format(sql.Identifier(schema)))
    try:
        with patch.object(balance,'connect',connect),patch('notifications.load_site_name',return_value='示例网关'):
            balance.initialize()
            ...
            body=dict(budget='100',threshold='10',enabled=True,start_month=first.strftime('%Y-%m'),version=1)
            calls=[]
            def source(months,now):
                if months:calls.append(months)
                return {m:Decimal(45) for m in months}
            with patch('balance.source_amounts',source):
                balance.save_settings(body,'test_admin')
            assert calls==[[first,previous]],calls
            balance.check_once(when,source,daily=False)
            with connect() as conn:
                assert conn.execute('SELECT count(*) AS n FROM balance_daily_runs').fetchone()['n']==0
            calls.clear()
            ...
            with connect() as conn:
                assert conn.execute('SELECT count(*) AS n FROM balance_months').fetchone()['n']==2
                assert conn.execute('SELECT count(*) AS n FROM balance_alerts').fetchone()['n']==1
                state=conn.execute('SELECT * FROM balance_state').fetchone()
                assert state['remaining']==Decimal(-35) and state['archived_amount']==90
            ...
            balance.check_once(when,source)
            with connect() as conn:
                assert conn.execute('SELECT resolved_at FROM balance_alerts').fetchone()['resolved_at'] is None
            balance.check_once(when+timedelta(days=1),source)
            with connect() as conn:
                assert conn.execute('SELECT count(*) AS n FROM balance_alerts').fetchone()['n']==0
            ...
            def race(months,now):
                with connect() as conn:
                    conn.execute('UPDATE balance_settings SET version=5,budget=500 WHERE id=1')
            ...
            try:
                ...
                pass
            with connect() as conn:
                assert conn.execute('SELECT version FROM balance_state').fetchone()['version']==4
            balance.record_failure()
            assert balance.snapshot()['stale']
            rollover=datetime.combine(balance.next_month(current),datetime.min.time(),tzinfo=TZ)
            ...
            with connect() as conn:
                conn.execute('CREATE TABLE logs(created_at bigint,type integer,quota bigint)')
                for moment,kind,quota in [(datetime(2025,12,31,23,59,59,tzinfo=TZ),2,500000),
                                           (datetime(2026,1,1,tzinfo=TZ),2,1000000),
                                           (datetime(2026,1,1,tzinfo=TZ),1,9000000),
                                           (datetime(2026,2,1,tzinfo=TZ),2,1500000)]:
                    conn.execute('INSERT INTO logs VALUES (%s,%s,%s)',(int(moment.timestamp()),kind,quota))
            with patch('balance.psycopg.connect',connect):
                amounts=balance.source_amounts([date(2025,12,1),date(2026,1,1),date(2026,2,1)],datetime(2026,2,1,tzinfo=TZ))
            assert amounts=={date(2025,12,1):Decimal(1),date(2026,1,1):Decimal(2),date(2026,2,1):Decimal(3)}
            # Simulate an older installation with multiple resolved alerts and reads.
            with connect() as conn:
                latest=conn.execute('INSERT INTO balance_alerts(remaining,threshold,spent,budget) '
                                    'VALUES (0,1,10,10) ON CONFLICT ((true)) '
                                    'DO UPDATE SET updated_at=now() RETURNING id').fetchone()['id']
                conn.execute('DROP INDEX one_balance_alert')
                conn.execute('CREATE TABLE balance_alert_reads (alert_id bigint REFERENCES balance_alerts(id), '
                             'username text,read_at timestamptz DEFAULT now())')
                for _ in range(2):
                    old=conn.execute("INSERT INTO balance_alerts(remaining,threshold,spent,budget,updated_at,resolved_at) "
                                     "VALUES (0,1,10,10,'2000-01-01','2000-01-02') RETURNING id").fetchone()['id']
                    conn.execute("INSERT INTO balance_alert_reads(alert_id,username) VALUES (%s,'old_reader')",(old,))
            balance.initialize()
            balance.initialize()
            with connect() as conn:
                remaining=conn.execute('SELECT id FROM balance_alerts').fetchall()
                assert remaining==[{'id':latest}]
                assert conn.execute("SELECT to_regclass('balance_alert_reads') AS name").fetchone()['name'] is None
            ...
            with patch.dict(os.environ,NOTIFICATION_ENCRYPTION_KEY=Fernet.generate_key().decode()), \
                ...
                config=dict(version=1,enabled=True,channel='feishu_app',app_id='cli_fixture',
                            app_secret='fixture-secret',robot_code='',
                            receive_id_type='user_id',receive_id='employee123')
                ...
                with connect() as conn:
                    cipher=conn.execute('SELECT secret_encrypted FROM notification_feishu_settings').fetchone()['secret_encrypted']
                    assert cipher!='fixture-secret' and notifications.decrypt(cipher)=='fixture-secret'
                    ...
                    assert not {'app_id','secret_encrypted','robot_code','receive_id_type','receive_id'} & columns
                    assert conn.execute('SELECT client_id,robot_code,open_conversation_id '
                                        'FROM notification_dingtalk_settings WHERE id=1').fetchone()=={
                                            'client_id':'','robot_code':'','open_conversation_id':''}
                notifications.save(dict(config,version=2,app_secret=''),'fixture_admin')
                notifications.deliver(test=True,expected_version=3)
                assert send.call_count==1 and send.call_args.args[1]=='fixture-secret'
                ...
                with connect() as conn:
                    conn.execute('UPDATE balance_settings SET budget=1,threshold=10 WHERE id=1')
                balance.check_once(when,source,daily=False)
                ...
                notifications.save(dict(config,version=3,enabled=False,app_secret=''),'fixture_admin')
                notifications.deliver()
                assert send.call_count==before
                try:
                    notifications.save(dict(config,version=4,app_id='cli_changed',app_secret=''),'fixture_admin')
                ...
                with connect() as conn:
                    conn.execute("UPDATE notification_feishu_settings SET receive_id_type='open_id', "
                                 "receive_id='ou_old' WHERE id=1")
                    conn.execute("UPDATE notification_settings SET enabled=true WHERE id=1")
                balance.initialize()
                migrated=notifications.snapshot()
                assert migrated['receive_id_type']=='user_id' and migrated['receive_id']==''
            ...
            print('PASS: encrypted credentials, secret retention, stale versions, test delivery, failures, recovery')
        print('PASS: immediate backfill, one check per day, live current month only, archive reuse, alerts, races, SQL boundaries')
    finally:
        with original_connect(dsn) as conn:
