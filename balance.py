...
def connect():
    ...
    return psycopg.connect(os.environ['MONITOR_DATABASE_URL'], connect_timeout=8,
...
def initialize():
    with connect() as conn:
        ...
        conn.execute(Path(__file__).with_name('balance.sql').read_text())
        conn.execute('INSERT INTO balance_settings(id,start_month) VALUES (1,%s) ON CONFLICT DO NOTHING',
...
def validate_settings(body, today=None):
    ...
    for key in ('budget', 'threshold'):
        ...
        if not value.is_finite() or not 0 <= value <= Decimal('999999999999') or value != value.quantize(Decimal('.01')):
    ...
    if not date(1970, 1, 1) <= month <= (today or datetime.now(TZ).date()).replace(day=1):
        raise ValueError('累计起始月份不能晚于本月。')
    if type(body.get('version')) is not int or body['version'] < 1:
        raise ValueError('设置版本无效，请重新打开设置。')
    return dict(values, start_month=month, enabled=body['enabled'], version=body['version'])
...
def save_settings(body, username):
    ...
    with connect() as conn:
        conn.execute('SELECT pg_advisory_xact_lock(90216321)')
        row = conn.execute('''UPDATE balance_settings SET budget=%(budget)s,
            threshold=%(threshold)s,start_month=%(start_month)s,enabled=%(enabled)s,
            version=version+1,updated_at=now(),updated_by=%(username)s
            WHERE id=1 AND version=%(version)s RETURNING *''', dict(values, username=username)).fetchone()
        if not row:
            raise SettingsConflict()
        conn.execute('''INSERT INTO balance_settings_audit(username,version,budget,threshold,start_month,enabled)
            VALUES (%s,%s,%s,%s,%s,%s)''',
                     (username,row['version'],row['budget'],row['threshold'],row['start_month'],row['enabled']))
        if not row['enabled']:
            conn.execute('UPDATE balance_alerts SET resolved_at=now(),updated_at=now() WHERE resolved_at IS NULL')
...
def archive_missing(conn, first, now):
    ...
    archived={r['month'] for r in conn.execute('SELECT month FROM balance_months').fetchall()}
    missing=[m for m in month_list(first,current) if m not in archived]
    if not missing:
        return
    for month,amount in source_amounts(missing,now).items():
        conn.execute('INSERT INTO balance_months(month,amount) VALUES (%s,%s) ON CONFLICT DO NOTHING',
                     (month,amount))
...
def source_amounts(months, now):
    ...
    for month in sorted(set(months)):
        ...
        params.extend([int(first.timestamp()),min(int(last.timestamp()),int(now.timestamp())+1)])
    with psycopg.connect(connect_timeout=8, row_factory=dict_row,
                         options='-c default_transaction_read_only=on -c statement_timeout=60000') as conn:
        rows = conn.execute('''SELECT date_trunc('month',to_timestamp(created_at)
            AT TIME ZONE 'Asia/Shanghai')::date AS month,
            SUM(quota::numeric)/500000 AS amount FROM logs
            WHERE type=2 AND ('''+' OR '.join(ranges)+') GROUP BY 1',params).fetchall()
...
def check_once(now=None, source=source_amounts, daily=True):
    ...
    current = now.date().replace(day=1)
    with connect() as conn:
        if not conn.execute('SELECT pg_try_advisory_xact_lock(90216321) AS locked').fetchone()['locked']:
            if not daily:
                raise CheckBusy()
            return
        ...
        if not settings or not settings['enabled']:
            return
        if daily and conn.execute('SELECT 1 FROM balance_daily_runs WHERE day=%s',(now.date(),)).fetchone():
            return
        archived = {r['month'] for r in conn.execute('SELECT month FROM balance_months').fetchall()}
        missing = [m for m in month_list(settings['start_month'],current) if m not in archived]
        amounts = source(missing+[current],now)
        # Do not publish a result calculated against settings changed mid-query.
        latest = conn.execute('SELECT version FROM balance_settings WHERE id=1 FOR UPDATE').fetchone()
        ...
        for month in missing:
            conn.execute('INSERT INTO balance_months(month,amount) VALUES (%s,%s) ON CONFLICT DO NOTHING',
                         (month,amounts[month]))
        historical = conn.execute('SELECT COALESCE(SUM(amount),0) AS amount FROM balance_months '
                                  'WHERE month >= %s AND month < %s',
                                  (settings['start_month'],current)).fetchone()['amount']
        spent = historical + amounts[current]
        remaining = settings['budget'] - spent
        conn.execute('''INSERT INTO balance_state(id,version,current_month,current_amount,archived_amount,
            ...
            remaining=EXCLUDED.remaining,checked_at=EXCLUDED.checked_at,last_error=NULL''',
                     (settings['version'],current,amounts[current],historical,remaining,now))
        if remaining < settings['threshold']:
            conn.execute('''INSERT INTO balance_alerts(remaining,threshold,spent,budget,updated_at)
        ...
        if daily:
            conn.execute('INSERT INTO balance_daily_runs(day) VALUES (%s) ON CONFLICT DO NOTHING',(now.date(),))
    # Commit the latest alert before contacting an external notification channel.
    from notifications import notify_safely
    notify_safely()
    return True
...
def record_failure():
    ...
    with connect() as conn:
        conn.execute("INSERT INTO balance_state(id,last_error) VALUES (1,'监控查询失败，请检查服务日志。') "
...
def snapshot(live=False):
    ...
    with connect() as conn:
        ...
        months = conn.execute('SELECT month,amount,archived_at FROM balance_months '
                              'WHERE month >= %s ORDER BY month DESC', (settings['start_month'],)).fetchall()
        alerts = conn.execute('SELECT * FROM balance_alerts ORDER BY updated_at DESC LIMIT 1').fetchall()
    now = datetime.now(TZ)
    if live:
        current=now.date().replace(day=1)
        ...
        if any(m not in archived for m in month_list(settings['start_month'],current)):
            return dict(configured=True,settings=settings,state=state,months=months,alerts=alerts,
                        valid=False,stale=True)
        amount=source_amounts([current],now)[current]
        historical=sum((m['amount'] for m in months),Decimal(0))
        state=dict(version=settings['version'],current_month=current,current_amount=amount,
                   archived_amount=historical,remaining=settings['budget']-historical-amount,
                   checked_at=now,last_error=None)
    valid = bool(state and state['checked_at'] and state['version']==settings['version']
                 and state['current_month']==now.date().replace(day=1))
    stale = not valid or (now-state['checked_at']).total_seconds()>90000 or bool(state['last_error'])
    return dict(configured=True,settings=settings,state=state,months=months,alerts=alerts,
