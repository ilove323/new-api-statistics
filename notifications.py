...
import balance
from notification_channels import CHANNELS
from notification_channels.base import DeliveryError
from report import load_site_name
...
def current_alert_record():
    ...
    return {
        ...
        'currency': 'CNY',
        'checked_at': alert['updated_at'].astimezone(balance.TZ).isoformat(timespec='seconds'),
...
def public(row):
    ...
    keys = ('version', 'enabled', 'channel', 'active_channel', 'app_id', 'robot_code', 'receive_id_type', 'receive_id',
            'last_attempt_at', 'last_success_at', 'last_error')
    return dict({k: row[k] for k in keys}, secret_configured=bool(row['secret_encrypted']))
...
def provider_config(conn, channel, lock=False):
    ...
    if channel == 'feishu_app':
        return conn.execute('''SELECT app_id,secret_encrypted,'' AS robot_code,
            receive_id_type,receive_id FROM notification_feishu_settings WHERE id=1''' + suffix).fetchone()
    if channel == 'dingtalk_app':
        return conn.execute('''SELECT client_id AS app_id,secret_encrypted,robot_code,
            'open_conversation_id' AS receive_id_type,
            open_conversation_id AS receive_id FROM notification_dingtalk_settings WHERE id=1''' + suffix).fetchone()
...
def snapshot(channel=None):
    ...
    if selected != row['channel']:
        result.update(enabled=False, last_attempt_at=None, last_success_at=None, last_error=None)
    return public(result)


def save(body, username):
    if not isinstance(body, dict) or type(body.get('enabled')) is not bool or type(body.get('version')) is not int:
    ...
    with balance.connect() as conn:
        row = conn.execute('SELECT * FROM notification_settings WHERE id=1 FOR UPDATE').fetchone()
        ...
        elif fields['receive_id_type'] not in {
                'feishu_app': ('chat_id', 'user_id'),
                'dingtalk_app': ('open_conversation_id',),
        }[channel]:
            raise ValueError('接收目标类型无效。')
        if channel == 'feishu_app':
            conn.execute('''UPDATE notification_feishu_settings SET app_id=%s,secret_encrypted=%s,
                receive_id_type=%s,receive_id=%s WHERE id=1''',
                         (fields['app_id'], encrypted, fields['receive_id_type'], fields['receive_id']))
        else:
            conn.execute('''UPDATE notification_dingtalk_settings SET client_id=%s,secret_encrypted=%s,
                robot_code=%s,open_conversation_id=%s WHERE id=1''',
                         (fields['app_id'], encrypted, fields['robot_code'], fields['receive_id']))
        conn.execute('''UPDATE notification_settings SET enabled=%s,channel=%s,version=version+1,
            updated_at=now(),updated_by=%s,last_attempt_at=NULL,last_success_at=NULL,last_error=NULL
            WHERE id=1''', (body['enabled'], channel, username))
...
def deliver(test=False, expected_version=None):
    ...
    with balance.connect() as conn:
        ...
        common = conn.execute('SELECT * FROM notification_settings WHERE id=1 FOR UPDATE').fetchone()
        config = dict(common)
        ...
        if test:
            ...
            alert = conn.execute('SELECT * FROM balance_alerts WHERE resolved_at IS NULL').fetchone()
            enabled = conn.execute('SELECT enabled FROM balance_settings WHERE id=1').fetchone()
        ...
        try:
            ...
            error = str(exc) if isinstance(exc, DeliveryError) else '通知发送失败，请检查服务配置。'
        conn.execute('''UPDATE notification_settings SET last_attempt_at=now(),last_error=%s,
