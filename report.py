...
HEADERS = ['用户名', '显示名', '模型', '总Token', '输入Token', '输出Token', '缓存读取Token', '缓存写入Token', '最后倍率', '输入价格（元/M）',
           '输出价格（元/M）', '缓存读价格（元/M）', '缓存写价格（元/M）',
           '消费金额（元）']
FIELDS = ['username', 'display_name', 'model_name', 'total_tokens', *TOKEN_FIELDS, 'group_ratio', 'input_price',
          'output_price', 'cache_price', 'write_price', 'amount', 'request_count']
...
def convert_tokens(row):
    ...
    row.update(converted_cache_read_tokens=None, converted_total_tokens=None,
    ...
    if ratio is None or ratio <= 0 or cache_price is None or cache_price <= 0:
        ...
        return
    fixed = Decimal(0)
    for token, price in [('pricing_input_tokens', 'input_price'),
                         ('output_tokens', 'output_price'), ('cache_write_tokens', 'write_price')]:
        count = row.get(token)
    ...
    if cache < 0 or total < 0:
        ...
        return
    row.update(converted_cache_read_tokens=cache, converted_total_tokens=total,
...
def parse_boundary(value, end=False):
    ...
    try:
        if len(value) == 10:
            return datetime.combine(date.fromisoformat(value), time(23, 59, 59) if end else time(), TZ)
...
def current_prices(model, options):
    ...
    for field, key in [('output_price', 'CompletionRatio'), ('cache_price', 'CacheRatio'),
                       ('write_price', 'CreateCacheRatio')]:
        multiplier = value(key)
        result[field] = base * multiplier if base is not None and multiplier is not None else None
...
def cost_formula(row):
    ...
    terms = []
    ratio = row['group_ratio']
    for label, token, price in [('输入', 'input_tokens', 'input_price'),
        ...
        weighted = Decimal(count) * unit if unit is not None else (Decimal(0) if count == 0 else None)
    ...
    return dict(terms=terms, ratio=ratio, calculated=calculated,
...
def decorate(rows, options):
    for row in rows:
        ...
        if row['converted_cache_read_tokens'] is not None:
            cache = int(row['converted_cache_read_tokens'].quantize(Decimal(1), rounding=ROUND_HALF_UP))
            row['converted_cache_read_tokens'] = cache
            row['converted_total_tokens'] = row['total_tokens'] - row['cache_read_tokens'] + cache
...
def load_site_name():
    ...
    with psycopg.connect(connect_timeout=8,
                         options='-c default_transaction_read_only=on -c statement_timeout=10000') as conn:
        row = conn.execute('SELECT value FROM options WHERE key = %s', ('SystemName',)).fetchone()
...
def totals(rows, start=None, end=None):
    ...
    result = {'total_tokens': tokens, 'amount': amount,
    ...
    if start is not None and end is not None:
        ...
        seconds = last - first
        result.update(duration_seconds=seconds,
...
def rankings(rows):
    ...
    for row in rows:
        ...
        user = users.setdefault(key, dict(user_id=key[0], username=key[1],
                                         display_name=row.get('display_name', ''),
                                         amount=Decimal(0), total_tokens=0,
                                         **dict.fromkeys(TOKEN_FIELDS, 0)))
        for field in ['amount', 'total_tokens', *TOKEN_FIELDS]:
            user[field] += row[field]
        models[row['model_name']] = models.get(row['model_name'], Decimal(0)) + row['amount']
    return {
        'model_amount': sorted((dict(model_name=k, amount=v) for k, v in models.items()),
                               key=lambda r: (-r['amount'], r['model_name'])),
        'user_tokens': sorted(users.values(), key=lambda r: (-r['total_tokens'], r['username'], r['user_id'])),
        'user_amount': sorted(users.values(), key=lambda r: (-r['amount'], r['username'], r['user_id'])),
    }
...
def export_excel(rows, start, end):
    ...
    ws.title = '用户模型用量'
    first, last = parse_boundary(start), parse_boundary(end, end=True)
    ws.append(['统计时间', f'{first:%Y-%m-%d %H:%M:%S} 至 {last:%Y-%m-%d %H:%M:%S}（北京时间）'])
    ...
    for i in range(1, len(rows) + 1):
        if i == len(rows) or (rows[i]['user_id'], rows[i]['username']) != (rows[i-1]['user_id'], rows[i-1]['username']):
            if i + 2 > begin:
                ws.merge_cells(start_row=begin, end_row=i + 2, start_column=1, end_column=1)
                ws.merge_cells(start_row=begin, end_row=i + 2, start_column=3, end_column=3)
            begin = i + 3
    for cells in ws.iter_rows(min_row=3):
        for c in cells:
            c.alignment = Alignment(vertical='center')
            if c.column not in (1, 3, 4):
                c.number_format = '#,##0' if c.column in (2, 5, 6, 7, 8, 9) else '#,##0.######'
    for c in ws[2]:
        c.font = Font(bold=True, color='FFFFFF')
        c.fill = PatternFill('solid', fgColor='156B59')
    for i, width in enumerate([24, 18, 24, 36, 30, 22, 22, 30, 22, 12, 24, 24, 24, 24, 22], 1):
        ws.column_dimensions[get_column_letter(i)].width = width
    ws.freeze_panes = 'E3'
    ws.auto_filter.ref = f'A2:O{last}'
    ranked = rankings(rows)
    for title, key, columns in [
        ('模型消费', 'model_amount', [('模型', 'model_name'), ('消费金额（元）', 'amount')]),
        ('用户Token用量', 'user_tokens', [('用户名', 'username'), ('显示名', 'display_name'), ('总Token', 'total_tokens'),
            ('输入Token', 'input_tokens'), ('输出Token', 'output_tokens'),
            ('缓存读取Token', 'cache_read_tokens'), ('缓存写入Token', 'cache_write_tokens')]),
        ('用户消费', 'user_amount', [('用户名', 'username'), ('显示名', 'display_name'), ('消费金额（元）', 'amount')]),
    ]:
        ...
        for col, (_, field) in enumerate(columns, 2):
            if field not in ('username', 'display_name'):
                letter = get_column_letter(col)
                sheet.cell(end_row + 1, col, f'=SUM({letter}3:{letter}{end_row})' if ranked[key] else 0)
        for col, (_, field) in enumerate(columns, 2):
            sheet.column_dimensions[get_column_letter(col)].width = 36 if field == 'username' else 24
            for line in range(3, sheet.max_row + 1):
                sheet.cell(line, col).number_format = '#,##0.######' if field == 'amount' else '#,##0'
        for c in sheet[2]:
            c.font = Font(bold=True, color='FFFFFF')
            c.fill = PatternFill('solid', fgColor='156B59')
    ...
    for title, col in [('消费金额（元）', 'O'), ('总Token', 'E'),
                       ('输入Token', 'F'), ('输出Token', 'G'),
                       ('缓存读取Token', 'H'), ('缓存写入Token', 'I'),
                       ('消费请求数', 'B')]:
    ...
    for row in range(3, 13):
        summary.cell(row, 2).number_format = '#,##0' if row in (4,5,6,7,8,9) else '#,##0.######'
    for c in summary[2]:
        c.font = Font(bold=True, color='FFFFFF')
        c.fill = PatternFill('solid', fgColor='156B59')
