...
def _post(path, body, token=None):
    ...
    req = request.Request(BASE + path, data=json.dumps(body, ensure_ascii=False).encode(),
    ...
    try:
        ...
        raise DeliveryError('钉钉连接或响应异常' + detail + '，请检查网络和应用配置。') from None
    if not 200 <= status < 300:
        code = result.get('code') or result.get('errcode')
        suffix = f'，错误码 {code}' if isinstance(code, (str, int)) else ''
        raise DeliveryError(f'钉钉请求失败（HTTP {status}{suffix}），请检查应用凭证、权限、发布状态及群聊配置。')
    for key in ('code', 'errcode'):
        code = result.get(key)
        if code not in (None, 0, '0'):
            raise DeliveryError(f'钉钉发送失败，错误码 {code}；请检查应用凭证、权限、发布状态及群聊配置。')
...
def _access_token(app_id, secret):
    ...
    with _lock:
        ...
        if cached and cached[1] > time.monotonic():
            return cached[0]
        result = _post('/v1.0/oauth2/accessToken', {'appKey': app_id, 'appSecret': secret})
...
def send(config, secret, text):
    ...
    _post('/v1.0/robot/groupMessages/send', {
