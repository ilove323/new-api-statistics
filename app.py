#!/usr/bin/env python3
# Usage: PGHOST=... PGPASSWORD=... python app.py
...
from report import TZ, export_excel, load_report, totals, parse_boundary, rankings, load_site_name
...
def authenticate():
    ...
    if not auth or auth.type != 'basic' or not verify_admin(auth.username, auth.password):
        return ('请使用 New API 管理员用户名和密码登录。', 401,
...
def usage():
    ...
    return jsonify(start=start, end=end, rows=rows, totals=totals(rows, start, end), rankings=rankings(rows),
...
def export():
    ...
    return send_file(export_excel(rows, start, end), as_attachment=True,
...
def monitor_write_allowed():
    ...
    return (request.headers.get('X-Statistics-Request') == '1'
            and request.is_json and request.headers.get('Sec-Fetch-Site') != 'cross-site')
